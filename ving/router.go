...
func (group *RouterGroup) Static(relativePath, localPath string, listDir bool) {
	...
	handler := func(ctx *Context) error {
		...
		finalAbsPath := filepath.Join(localPath, filepath.FromSlash(relPath))
		...
		// Ensure the final path is within the permitted local path
		// We use filepath.Rel to check if the path is relative and doesn't contain ".."
		rel, err := filepath.Rel(localPath, finalAbsPath)
