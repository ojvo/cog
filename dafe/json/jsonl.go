...
//
// 用环形缓冲区流式读取，只保留最后 N 条记录，保持 O(N) 内存而非
// ReadAll 的 O(全部)。这与 ForEachLine 的流式设计一致，避免大
// JSONL 文件（如事件日志回放）OOM。
func (r *JSONLReader) ReadLast(path string, n int, factory func() any) ([]any, error) {
	...
	// 环形缓冲区：buf 容量 n，idx 指向下一个被覆盖的槽位。
	buf := make([]any, 0, n)
	idx := 0
	err = ForEachLine(f, func(line []byte) error {
		if len(line) == 0 || (len(line) == 1 && line[0] == '\n') {
	...
	// 已填满：按原始顺序重排（idx 是最旧记录的位置）
	result := make([]any, 0, n)
	for i := 0; i < n; i++ {
		result = append(result, buf[(idx+i)%n])
	}
	return result, nil
}
