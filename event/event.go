...
func NewEvent(eventType string, data interface{}) Event {
	return Event{
		...
		EventID:   fmt.Sprintf("evt_%s_%d", eventType, time.Now().UnixNano()),
	}
}
...
func (bus *EventHub) Publish(event Event) {
	...
	// 确保事件有唯一ID
	if event.EventID == "" {
		event.EventID = fmt.Sprintf("evt_%s_%d", event.Type, time.Now().UnixNano())
	}

	// 检查事件是否已处理过（根据配置决定是否启用）
	if bus.config.EnableEventDeduplication && bus.isEventProcessed(event.EventID) {
		bus.logf("DEBUG", "事件 %s (ID: %s) 已处理过，跳过", event.Type, event.EventID)
		return
	}

	// 标记事件已处理（仅当启用去重时）
	if bus.config.EnableEventDeduplication {
		bus.markEventProcessed(event.EventID)
	}
...
// 添加事件处理记录检查
func (bus *EventHub) isEventProcessed(eventID string) bool {
	bus.processedEvents.mu.RLock()
	defer bus.processedEvents.mu.RUnlock()

	return bus.processedEvents.cache[eventID]
}
...
func (bus *EventHub) executeHandlerWithTimeout(sub Subscription, event Event) error {
...
func (e Event) AsRequest() Event {
	e.RequestID = fmt.Sprintf("req_%s_%d", e.Type, time.Now().UnixNano())
	return e
}
...
func (bus *EventHub) Respond(req RequestEvent, success bool, data interface{}, err error) {
	...
	// 直接尝试发送响应，避免不必要的goroutine开销和调度延迟
	// 通道有缓冲，通常不会阻塞
	select {
	case req.ResponseChan <- resp:
		// 成功发送响应
	default:
		// 通道已满，尝试使用goroutine发送（虽然对于缓冲为1的通道，这意味着已经有一个响应了）
		go func() {
			// 使用defer-recover捕获可能的panic
			defer func() {
				if r := recover(); r != nil {
					bus.logf("ERROR", "发送响应时发生panic: %v", r)
				}
			}()
			...
			case req.ResponseChan <- resp:
				// 成功发送响应
			case <-time.After(1 * time.Second): // 避免永久阻塞
				bus.logf("ERROR", "无法发送响应，通道阻塞超时: %s", req.Event.Type)
			}
		}()
	}
}
...
// 标记事件已处理
func (bus *EventHub) markEventProcessed(eventID string) {
	...
	if len(bus.processedEvents.cache) >= bus.processedEvents.maxSize {
		// 简单清理策略：直接重置
		bus.processedEvents.cache = make(map[string]bool)
	}
