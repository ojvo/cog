...
func TestForEachLine_LargeLineExceedsInitialChunk(t *testing.T) {
	// 构造一条超过 initialChunk (64KiB) 的超长行，验证 buffer 动态扩容。
...
func TestForEachLine_ManySmallLines(t *testing.T) {
	// 构造大量短行，验证 SIMD 扫描和 buffer compact 前移逻辑。
...
func TestForEachLine_NoProgress(t *testing.T) {
	// 一个行为不良的 Reader，反复返回 (0, nil) 应触发 ErrNoProgress。
	r := &stuckReader{}
...
func TestForEachLine_PartialReads(t *testing.T) {
	// 模拟慢速 Reader，每次只返回少量字节，验证跨 Read 拼接逻辑。
	data := "hello\nworld\nfoo\nbar\n"
...
func TestForEachLine_MixedLargeAndSmall(t *testing.T) {
	// 混合大行和小行，验证扩容后 compact 前移正确处理后续小行。
...
// TestJSONLReader_ReadAll_SkipsCorruptLines verifies that corrupt lines are
// skipped (not fatal) so that as much valid data as possible is recovered.
func TestJSONLReader_ReadAll_SkipsCorruptLines(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "corrupt.jsonl")

	// Write a file with a valid line, a corrupt line, and a valid line.
	content := `{"n":1}
{corrupt json}
{"n":3}
`
	...
	// Should have 2 valid entries (n=1 and n=3), corrupt line skipped.
	if len(results) != 2 {
		t.Errorf("expected 2 valid entries, got %d", len(results))
	}
}

// TestJSONLReader_ReadAll_EmptyLines verifies that empty lines are skipped.
func TestJSONLReader_ReadAll_EmptyLines(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "empty.jsonl")

	// Write a file with valid lines separated by empty lines.
	content := `{"n":1}
...
// TestJSONLReader_ReadAll_NoTrailingNewline verifies that a file without a
// trailing newline is still read correctly (ForEachLine handles this).
