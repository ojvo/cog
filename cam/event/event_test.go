...
// TestEventHub_EventID_Unique verifies that EventIDs generated concurrently
// do not collide (regression for time.Now().UnixNano()-only IDs).
func TestEventHub_EventID_Unique(t *testing.T) {
	const n = 1000
	ids := make([]string, n)
	var wg sync.WaitGroup
	wg.Add(n)
	...
	for _, id := range ids {
		if _, dup := seen[id]; dup {
			t.Fatalf("duplicate EventID: %s", id)
		}
		seen[id] = struct{}{}
	}
}

// TestEventHub_TimeoutHandler_PanicRecovery verifies that a panicking handler
// in the timeout path is recovered into an error instead of crashing the test.
func TestEventHub_TimeoutHandler_PanicRecovery(t *testing.T) {
	config := DefaultEventHubConfig()
	config.UseWorkerPool = false
	bus := NewEventHub(config)
	defer bus.Close()
	...
	sub := Subscription{
		...
		Options: HandlerOptions{Timeout: time.Second},
	}
	// executeHandlerWithTimeout must return an error (from recover), not crash.
	err := bus.executeHandlerWithTimeout(sub, NewEvent("panic_evt", nil))
	if err == nil {
		t.Fatal("expected error from recovered panic, got nil")
	}
}

// TestEventHub_Dedup_NoFullWipe verifies that when the dedup cache exceeds
// maxSize, it does NOT wipe all entries (which would re-admit duplicates of
// recently-seen events). After overflow, most previously-seen IDs must still
// be recognized.
func TestEventHub_Dedup_NoFullWipe(t *testing.T) {
	config := DefaultEventHubConfig()
	config.EnableEventDeduplication = true
	bus := NewEventHub(config)
	defer bus.Close()

	// Force a small maxSize so we can trigger overflow without huge allocations.
	bus.processedEvents.mu.Lock()
	bus.processedEvents.maxSize = 20
	bus.processedEvents.mu.Unlock()

	// Mark 20 events as processed (fill to capacity).
	for i := 0; i < 20; i++ {
		bus.checkAndMarkEvent(fmt.Sprintf("id_%d", i))
	}
	// One more triggers eviction (10% = 2 entries removed), not a full wipe.
	bus.checkAndMarkEvent("id_overflow")

	// At least some of the originally-marked events should still be recognized
	// as processed. A full wipe would recognize NONE of them.
	recognized := 0
	for i := 0; i < 20; i++ {
		if bus.checkAndMarkEvent(fmt.Sprintf("id_%d", i)) {
			recognized++
		}
	}
	if recognized == 0 {
		t.Fatal("dedup cache was fully wiped on overflow; expected partial eviction")
	}
}
