...
//
// By default panics inside launched goroutines are silently recovered. Use
// OnPanic to install an observer (e.g. logger) before launching goroutines.
//
...
type Goroutine struct {
	...
	onPanic func(recover interface{}, stack []byte)
	mu      sync.Mutex
}
...
// OnPanic installs a handler invoked when a launched goroutine panics. The
// handler receives the recovered value and the goroutine stack trace. Pass nil
// to revert to the default silent-recover behavior. Must be called before Go.
// Returns g for chaining.
func (g *Goroutine) OnPanic(handler func(recover interface{}, stack []byte)) *Goroutine {
	g.mu.Lock()
	g.onPanic = handler
	g.mu.Unlock()
	return g
...
// SubGo launches f as a goroutine with a child context derived from the
// group's context. The returned CancelFunc cancels only this goroutine's
// context; other goroutines and the parent context are unaffected. The
// goroutine is tracked by the group (Wait/Done/Active observe it) and its
// panic is handled by OnPanic if set.
//
// Useful for launching sub-tasks that can be cancelled individually without
// stopping the whole group.
func (g *Goroutine) SubGo(f func(ctx context.Context)) context.CancelFunc {
	if f == nil {
		panic("syncx: SubGo func is nil")
	}
	subCtx, cancel := context.WithCancel(g.ctx)
	...
	go g.runWith(subCtx, f)
	return cancel
}

func (g *Goroutine) runWith(ctx context.Context, f func(ctx context.Context)) {
	defer func() {
		if r := recover(); r != nil {
			g.mu.Lock()
			handler := g.onPanic
