...
// HTTPStatusError reports an HTTP response that RetryHTTP could not treat as
// a successful result. It is returned both for non-retryable HTTP failures and
// when retries for a retryable status are exhausted.
...
func RetryHTTP(method string, operation func() (int, error), config *HTTPRetryConfig) error {
	...
	for attempt := 0; attempt <= config.MaxRetries; attempt++ {
		...
		// Only HTTP statuses below 400 are successful. A non-success response
		// that is not retryable must remain visible to the caller instead of
		// being silently converted to nil.
		if err == nil && statusCode < 400 {
			return nil
		}
		if err == nil && !ShouldRetryHTTP(method, statusCode, attempt, config) {
			return &HTTPStatusError{StatusCode: statusCode}
		}

		// If the retry budget is exhausted, preserve either the operation error
		// or the final HTTP failure.
		if attempt >= config.MaxRetries {
			if err != nil {
				return err
			}
